...
def split_geojson_parts(input_path, output_dir, max_size_mb=MAX_PART_SIZE_MB):
        ...
        with tempfile.TemporaryDirectory(prefix="geojson_split_") as temp_dir:
            try:
                for file_path in self.selected_files:
                    ...
                    created_files = split_geojson_parts(file_path, temp_dir, max_size_mb=max_size_mb)
                    all_created_files.extend(created_files)
                    self.append_status(f"  -> Tao {len(created_files)} file")

                zip_file = build_zip(zip_path, all_created_files)
