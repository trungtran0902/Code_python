...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        input_file = filedialog.askopenfilename(
            title="Chọn file OSM PBF (hoặc PBF)",
            filetypes=[("OSM PBF Files", "*.osm.pbf *.pbf"), ("All Files", "*.*")]
        )
        if not input_file:
            print("Chưa chọn file đầu vào. Thoát chương trình.")
            return

        # Tùy chọn cho phép người dùng chọn nơi lưu file xuất ra
        output_file = filedialog.asksaveasfilename(
            title="Lưu file kết quả txt",
            defaultextension=".txt",
            filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")],
            initialfile="out.txt"
        )
        if not output_file:
            print("Chưa chọn nơi lưu file đầu ra. Mặc định lưu vào thư mục hiện tại với tên out.txt")
            output_file = "out.txt"
    ...
    print(f"Hoàn thành. Phát hiện {len(results)} lỗi. Đã lưu báo cáo tại -> {output_file}")
