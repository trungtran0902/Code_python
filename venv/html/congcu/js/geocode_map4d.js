...
function applyCheckpointData(checkpoint) {
  ...
  fileInfo.textContent = normalizeText(`Da phuc hoi sheet "${sheetName}" Ã¢â‚¬â€ ${rows.length} rows`);
  logEl.textContent = checkpoint.logText || "";
  refreshProgress();
  downloadBtn.disabled = processedCount === 0;
  runBtn.textContent = hasCompletedRun ? "Chay lai" : (currentRowIndex > 0 ? "Tiep tuc" : "Thuc Hien");
  updateCheckpointInfo(
...
fileInput.onchange = e => {
  ...
  fileInfo.textContent = normalizeText(`Loaded sheet "${sheetName}" Ã¢â‚¬â€ ${rows.length} rows`);
  totalRows.textContent = rows.length;
  processedCount = 0;
  successCount = 0;
  errorCountValue = 0;
...
runBtn.onclick = async () => {
  ...
  if (mode === "forward" && !fcols.length) {
    return alert("ChÃ¡Â»Ân ÃƒÂ­t nhÃ¡ÂºÂ¥t 1 cÃ¡Â»â„¢t Ã„â€˜Ã¡Â»â€¹a chÃ¡Â»â€°");
  }
  ...
  runBtn.textContent = currentRowIndex === 0 ? "Dang chay..." : "Tiep tuc";
  pauseBtn.disabled = false;
  pauseBtn.textContent = "Tam ngung";
  downloadBtn.disabled = processedCount === 0;
  ...
  for (let i = currentRowIndex; i < rows.length; i += REQUEST_CONCURRENCY) {
    if (isPaused) {
      ...
      runBtn.textContent = "TiÃ¡ÂºÂ¿p tÃ¡Â»Â¥c";
      pauseBtn.disabled = false;
      pauseBtn.textContent = "Tiep tuc";
      downloadBtn.disabled = processedCount === 0;
      log("Ã¢ÂÂ¸Ã¯Â¸Â Ã„ÂÃƒÂ£ tÃ¡ÂºÂ¡m ngÃ†Â°ng. BÃ¡ÂºÂ¡n cÃƒÂ³ thÃ¡Â»Æ’ tÃ¡ÂºÂ£i kÃ¡ÂºÂ¿t quÃ¡ÂºÂ£ tÃ¡ÂºÂ¡m.");
...
pauseBtn.onclick = async () => {
  if (isRunning) {
    isPaused = true;
    pauseBtn.disabled = true;
    pauseBtn.textContent = "Dang tam ngung...";
    return;
  }
...
drawBtn.onclick = async () => {
  ...
  log(`Ã°Å¸â€”ÂºÃ¯Â¸Â Marker: ${markerCount} | POI: ${poiCount}`);
};
