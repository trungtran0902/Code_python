...
function buildGoogleMapsDirectionsUrl(origin, destination) {
  ...
  const googleMode = mode === "foot" ? "walking" : mode === "bike" ? "bicycling" : "driving";
  const params = new URLSearchParams({
    api: "1",
    origin: formatLatLng(origin),
    destination: formatLatLng(destination),
  ...
  return `https://www.google.com/maps/dir/?${params.toString()}`;
    ...
    let overlayCenter = { lat: 10.776889, lng: 106.700806 }, overlayZoom = 14, mapListenersAttached = false, overlayRefreshScheduled = false, googleTileRefreshScheduled = false, mapClickAttached = false;
    ...
    const routeOverlay = () => $("routeOverlay"), routeSvgPath = () => $("routeSvgPath"), routeShadow = () => $("routeShadow"), domPointA = () => $("domPointA"), domPointB = () => $("domPointB"), domCar = () => $("domCar"), googleTileLayer = () => $("googleTileLayer"), mapPanel = () => $("mapPanel");
    ...
    function panToPoint(point) { try { if (map?.panTo) map.panTo(point, { duration: 450, animate: true }); else if (map?.moveCamera) map.moveCamera({ target: point }, { duration: 450, animate: true }); } catch {} setTimeout(refreshAllMapOverlays, 450); }
...
Đang gọi Route API để vẽ tuyến...`);
    ...
    function applyRouteResult(result, options = {}) {
      ...
      updateGoogleMapsDirectionsPanel(origin, destination, result.originLabel || "Điểm A", result.destinationLabel || "Điểm B");
...
${analysis.issues.join("\n")}`, "error");
    ...
    function updateGoogleMapsDirectionsPanel(origin, destination, originLabel = "Điểm A", destinationLabel = "Điểm B") {
      const panel = $("googleMapsPanel"), openBtn = $("openGoogleMapsBtn"), copyBtn = $("copyGoogleMapsBtn");
      if (!panel || !openBtn || !copyBtn) return;
      if (!origin || !destination) {
        currentGoogleMapsUrl = "";
        panel.className = "google-panel google-empty";
        panel.textContent = "Sau khi vẽ tuyến Map4D, hệ thống sẽ tự tạo link Google Maps Directions cho cùng cặp A/B.";
        openBtn.disabled = true;
        copyBtn.disabled = true;
        return;
      }
      currentGoogleMapsUrl = buildGoogleMapsDirectionsUrl(origin, destination);
      const travelText = getGoogleTravelModeText();
      panel.className = "google-panel google-ready";
      panel.innerHTML = `Đã tạo link Google Maps Directions cho cùng cặp A/B.
A: ${escapeHtml(originLabel)} (${formatLatLng(origin)})
B: ${escapeHtml(destinationLabel)} (${formatLatLng(destination)})
Chế độ Google: ${travelText}<span class="google-url">${escapeHtml(currentGoogleMapsUrl)}</span>`;
      openBtn.disabled = false;
      copyBtn.disabled = false;
    }

    function getGoogleTravelModeText() {
      const mode = $("modeInput")?.value || "car";
      if (mode === "foot") return "walking / đi bộ";
      if (mode === "bike") return "bicycling / xe đạp";
      if (mode === "motorcycle") return "driving / lái xe";
      return "driving / lái xe";
    }

    function openGoogleMapsDirections() {
      if (!currentGoogleMapsUrl) {
        alert("Chưa có link Google Maps. Hãy tìm đường hoặc random tuyến trước.");
        return;
      }
      window.open(currentGoogleMapsUrl, "_blank", "noopener,noreferrer");
    }

    async function copyGoogleMapsDirectionsLink() {
      if (!currentGoogleMapsUrl) {
        alert("Chưa có link Google Maps. Hãy tìm đường hoặc random tuyến trước.");
        return;
      }
      try {
        ...
        logStatus("Đã copy link Google Maps Directions vào clipboard.", "success");
      } catch (error) {
        console.warn("Clipboard API failed", error);
        const temp = document.createElement("textarea");
        temp.value = currentGoogleMapsUrl;
    ...
    function setupGoogleTileControls() {
      const layerInput = $("googleTileLayerInput");
      const opacityInput = $("googleTileOpacityInput");
      const opacityText = $("googleTileOpacityText");
      const updateOpacityText = () => {
        if (!opacityInput || !opacityText) return;
        opacityText.textContent = `${Math.round(Number(opacityInput.value || 0.9) * 100)}%`;
      };
      if (layerInput) layerInput.addEventListener("change", () => refreshGoogleTileOverlay(true));
      if (opacityInput) opacityInput.addEventListener("input", () => { updateOpacityText(); refreshGoogleTileOverlay(true); });
      updateOpacityText();
    }
    function getSelectedGoogleTileLayer() {
      return $("googleTileLayerInput")?.value || "none";
    }
    function getGoogleTileOpacity() {
      const v = Number($("googleTileOpacityInput")?.value || 0.9);
      return Number.isFinite(v) ? clamp(v, 0.2, 1) : 0.9;
    }
    function buildGoogleTileUrl(template, x, y, z) {
      return template.replace("{x}", String(x)).replace("{y}", String(y)).replace("{z}", String(z));
    }
    function getMapZoom() {
      try { if (map?.getZoom) { const z = Number(map.getZoom()); if (Number.isFinite(z)) return z; } } catch {}
      try { if (map?.getCamera) { const camera = map.getCamera(); const z = Number(camera?.zoom); if (Number.isFinite(z)) return z; } } catch {}
      return overlayZoom || 14;
    }
    function scheduleGoogleTileRefresh() {
      if (googleTileRefreshScheduled) return;
      googleTileRefreshScheduled = true;
      requestAnimationFrame(() => {
        googleTileRefreshScheduled = false;
        refreshGoogleTileOverlay(false);
      });
    }
    function refreshGoogleTileOverlay(force = false) {
      const layerEl = googleTileLayer();
      if (!layerEl) return;
      const selected = getSelectedGoogleTileLayer();
      const template = GOOGLE_TILE_LAYER_URLS[selected] || "";
      if (!template) {
      ...
      if (!panel) return;
      const width = Math.max(320, panel.clientWidth || 0);
      const height = Math.max(320, panel.clientHeight || 0);
      const center = getMapCenter() || overlayCenter || parseLatLngLoose(DEFAULT_AUTOSUGGEST_LOCATION);
      const z = clamp(Math.round(getMapZoom()), 1, 22);
      const tilesAtZoom = Math.pow(2, z);
      const scale = 256 * tilesAtZoom;
      ...
      const opacity = getGoogleTileOpacity();
      const cacheKey = `${selected}|${z}|${Math.round(topLeft.x)}|${Math.round(topLeft.y)}|${width}|${height}|${opacity}`;
      if (!force && layerEl.dataset.cacheKey === cacheKey) {
        layerEl.style.opacity = String(opacity);
        layerEl.classList.add("active");
        return;
      }
      layerEl.dataset.cacheKey = cacheKey;
      layerEl.style.opacity = String(opacity);
      layerEl.innerHTML = "";
      ...
      for (let tx = minTileX; tx <= maxTileX; tx++) {
        ...
        for (let ty = minTileY; ty <= maxTileY; ty++) {
          ...
          img.src = buildGoogleTileUrl(template, wrappedX, ty, z);
          img.style.left = `${Math.round(tx * 256 - topLeft.x)}px`;
          img.style.top = `${Math.round(ty * 256 - topLeft.y)}px`;
    ...
    function refreshAllMapOverlays() {
    ...
    function setViewportToFitPath(path) {
      ...
      setTimeout(refreshAllMapOverlays, 80); setTimeout(refreshAllMapOverlays, 520);
    }
    ...
    function resetMapObjects(clearLog = true) { pauseSimulation(); safeRemove(routeLine); safeRemove(originMarker); safeRemove(destinationMarker); safeRemove(carMarker); routeLine = originMarker = destinationMarker = carMarker = null; routePath = []; segments = []; totalDistance = 0; startTime = null; paused = false; pausedAt = 0; totalPausedTime = 0; lastDistanceTravelled = 0; currentGoogleMapsUrl = ""; clearDomOverlay(); updateStats(0, 0); updateRouteQualityPanel(null); updateGoogleMapsDirectionsPanel(null, null); setStateText("Chưa chạy"); if (clearLog) logStatus("Đã xóa tuyến và reset mô phỏng. Điểm A/B đã chọn vẫn được giữ nguyên."); }
    ...
    function attachMapListeners() { if (!map || mapListenersAttached || !map.addListener) return; mapListenersAttached = true; const events = []; if (map4d.MapEvent) events.push(map4d.MapEvent.cameraChanging, map4d.MapEvent.idle, map4d.MapEvent.drag, map4d.MapEvent.dragEnd, map4d.MapEvent.boundsChanged, map4d.MapEvent.zoomChanged); events.push("cameraChanging", "idle", "drag", "dragEnd", "boundsChanged", "zoomChanged"); [...new Set(events.filter(e => e !== undefined && e !== null))].forEach(e => { try { map.addListener(e, refreshAllMapOverlays); } catch {} }); }
    ...
    window.addEventListener("resize", refreshAllMapOverlays);
