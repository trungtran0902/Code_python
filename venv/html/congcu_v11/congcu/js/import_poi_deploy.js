...
function writeImportActionLog() {
    ...
    if (!store?.addLog) {
        console.warn('Map4DLogStore chưa sẵn sàng, không thể ghi log Import POI.');
        return null;
    }
    ...
    const message = `Import file ${areaLabel} ${Number(total || 0).toLocaleString('vi-VN')} POI`;

    const logItem = store.addLog({
        action: 'import_poi',
        actionLabel: 'Import POI',
        area,
        rows: sourceRows,
        poiCount: total,
        successCount,
    ...
    if (logItem) console.log('Đã ghi log Import POI vào Dashboard:', logItem);
    return logItem;
}
...
async function processQueue() {
    ...
    if (processState.processedIndices.size >= processState.total) {
        processState.status = 'completed'; saveCheckpoint(); updateProgressUI(true); writeImportActionLog(); Swal.fire('Hoàn thành', 'Xử lý xong! Log Import POI đã được ghi vào Dashboard. Bạn có thể bấm Kiểm tra ID để rà lại các ID đã import.', 'success');
