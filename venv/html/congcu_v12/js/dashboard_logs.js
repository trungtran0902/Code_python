...
function renderStats(logs) {
  ...
  $('statTotalRows').textContent = formatNumber(logs.reduce((sum, log) => sum + Number(log.poiCount || 0), 0));
  $('statSuccessRows').textContent = formatNumber(logs.reduce((sum, log) => sum + Number(log.successCount || 0), 0));
  $('statErrorRows').textContent = formatNumber(logs.reduce((sum, log) => sum + Number(log.errorCount || 0), 0));
}
...
function renderTimeline(logs) {
  ...
  if (!logs.length) {
    container.innerHTML = '<div class="empty">Chưa có log. Bạn có thể bấm “Tạo log mẫu” hoặc chạy Import POI / Del POI / Geocode để tự ghi log.</div>';
    return;
  }
    ...
    return `
      <article class="day-group">
        <header class="day-header">
          ...
          <span>${formatNumber(items.length)} log · ${formatNumber(totalPoi)} dòng</span>
        </header>
        <div class="log-list">
          ${items.map(log => `
            <div class="log-row">
              ...
              <div class="count">${formatNumber(log.poiCount)} dòng</div>
              <div class="msg">
...
function addManualLog() {
  ...
  const message = $('manualMessage').value.trim() || `${Map4DLogStore.actionLabel(action)} ${area} ${formatNumber(poiCount)} dòng`;
...
function setupRealtimeLogRefresh() {
  window.addEventListener('map4d-log-updated', render);
  window.addEventListener('storage', (event) => {
    if (!event.key || event.key === Map4DLogStore.STORAGE_KEY) render();
  });
  window.addEventListener('message', (event) => {
    if (event?.data?.type === 'map4d-log-updated') render();
  });
  try {
    const channel = new BroadcastChannel('map4d-tools-log-channel');
