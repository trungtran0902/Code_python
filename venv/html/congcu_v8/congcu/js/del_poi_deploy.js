...
if(btnDeleteManual) btnDeleteManual.addEventListener('click', async () => {
    ...
    if (result.success) {
        Swal.fire('Thành công', `Đã xóa POI: ${id}\nLog Delete POI đã được ghi vào Dashboard.`, 'success');
        manualIdInput.value = '';
    } else {
        Swal.fire('Thất bại', `Lỗi xóa POI: ${result.message}\nLog lỗi Delete POI đã được ghi vào Dashboard.`, 'error');
    }
});
...
function addDashboardLogSafe(payload) {
    ...
    if (!store?.addLog) {
        console.warn('Map4DLogStore chưa sẵn sàng, không thể ghi Dashboard Log.', payload);
        return null;
    }
    return store.addLog(payload);
}
...
function writeDeleteActionLog() {
    ...
    const store = window.Map4DLogStore;
    const sourceRows = candidateRows.map(item => item.rowData || item);
    const fileName = processState.sourceFileName || fileInput?.files?.[0]?.name || '';
    const area = store?.inferAreaFromFileAndRows
        ? store.inferAreaFromFileAndRows(fileName, sourceRows)
        : (store?.inferAreaFromText ? store.inferAreaFromText(fileName) : 'N/A');

    const total = processState.total || candidateRows.length || processState.results.length || 0;
    const successCount = processState.results.filter(r => r.status === 'success').length;
    const errorCount = Math.max(0, total - successCount);
    const areaLabel = area && area !== 'N/A' ? area : 'N/A';
    const message = `Del POI ${areaLabel} ${Number(total || 0).toLocaleString('vi-VN')} POI`;

    const logItem = addDashboardLogSafe({
        action: 'delete_poi',
        actionLabel: 'Delete POI',
        area,
        rows: sourceRows,
        ...
        source: 'del_poi_deploy',
        message,
        meta: {
            processedCount: processState.processedIndices.size,
            idColumn: idColumnSelect?.value || '',
            completedAt: new Date().toISOString()
        }
    });

    processState.__actionLogWritten = true;
    if (logItem) console.log('Đã ghi log Delete POI vào Dashboard:', logItem);
    return logItem;
}
...
async function processQueue() {
    ...
    if (processState.processedIndices.size >= processState.total) {
        ...
        Swal.fire('Hoàn thành', 'Đã xử lý xong tất cả. Log Delete POI đã được ghi vào Dashboard.', 'success');
