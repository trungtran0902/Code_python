...
const PLACE_DETAIL_OUTPUT_FIELDS = [
  "detail_id", "detail_name", "detail_address", "detail_oldaddress",
  "detail_lat", "detail_lng", "detail_phone", "detail_website",
  "detail_types", "detail_status", "detail_error"
];

const MODE_OUTPUT_FIELDS = {
  forward: [
    "id", "name", "address", "oldaddress", "lat", "lng",
    ...PLACE_DETAIL_OUTPUT_FIELDS,
    "status", "error"
  ],
  reverse: [
    "id", "name", "address", "oldaddress", "lat", "lng",
...
function setPlaceDetailOutput(row, mode, detailPlace, detailStatus, detailError) {
  const normalized = normalizePlaceInfo(detailPlace) || {};
  setOutputValue(row, mode, "detail_id", normalized.id || "");
  setOutputValue(row, mode, "detail_name", normalized.name || "");
  setOutputValue(row, mode, "detail_address", normalized.address || "");
  setOutputValue(row, mode, "detail_oldaddress", normalized.oldaddress || "");
  setOutputValue(row, mode, "detail_lat", normalized.lat ?? "");
  setOutputValue(row, mode, "detail_lng", normalized.lng ?? "");
  setOutputValue(row, mode, "detail_phone", normalized.phone || "");
  setOutputValue(row, mode, "detail_website", normalized.website || "");
  setOutputValue(row, mode, "detail_types", normalized.types || "");
  setOutputValue(row, mode, "detail_status", detailStatus || "");
  setOutputValue(row, mode, "detail_error", detailError || "");
}

async function enrichWithPlaceDetail(place, key) {
  const base = normalizePlaceInfo(place) || {};
  const detailResult = await placeDetailSafe(base.id, key);
  return {
    place: mergePlaceInfo(place, detailResult.place),
    detailPlace: detailResult.place,
...
async function processRowAtIndex(i, key, mode) {
  ...
  try {
    ...
    if (mode === "forward") {
      ...
      const enriched = await enrichWithPlaceDetail(r, key);
      setCommonPlaceOutput(row, mode, enriched.place);
      setPlaceDetailOutput(row, mode, enriched.detailPlace, enriched.detailStatus, enriched.detailError);
      setOutputValue(row, mode, "status", "OK");
      setOutputValue(row, mode, "error", "");

      if (enriched.detailStatus === "OK") {
        log(`Row ${i + 1}: Place Detail OK - ${getOutputValue(row, mode, "name")} - ${getOutputValue(row, mode, "address")}`);
      } else {
        log(`Row ${i + 1}: ${getOutputValue(row, mode, "name")} - ${getOutputValue(row, mode, "address")} | Detail: ${enriched.detailStatus}`, "warn");
      }
      rowSucceeded = true;
    }
    ...
    if (mode === "nearby") {
      ...
      if (r.error) {
        ...
        const enriched = await enrichWithPlaceDetail(r.raw || r, key);
        setCommonPlaceOutput(row, mode, enriched.place);
        setOutputValue(row, mode, "place_id", enriched.place.id || r.id || "");
        setOutputValue(row, mode, "place_name", enriched.place.name || r.name || "");
        setOutputValue(row, mode, "place_address", enriched.place.address || r.address || "");
        ...
        if (enriched.detailStatus === "OK") {
          log(`Row ${i + 1}: Place Detail OK - ${getOutputValue(row, mode, "place_name")} - ${getOutputValue(row, mode, "place_address")}`);
        } else {
          log(`Row ${i + 1}: ${getOutputValue(row, mode, "place_name")} - ${getOutputValue(row, mode, "place_address")} | Detail: ${enriched.detailStatus}`, "warn");
        }
        rowSucceeded = true;
      }
    }
    ...
    if (mode === "reverse") {
      ...
      setCommonPlaceOutput(row, mode, {
        ...enriched.place,
        address: enriched.place.address || address
      });
      setPlaceDetailOutput(row, mode, enriched.detailPlace, enriched.detailStatus, enriched.detailError);
      setOutputValue(row, mode, "status", "OK");
      setOutputValue(row, mode, "error", "");
